<?
//a:9:{s:4:"lang";s:2:"cn";s:9:"auth_pass";s:32:"21232f297a57a5a743894a0e4a801fc3";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";s:0:"";s:7:"fm_root";s:0:"";s:17:"cookie_cache_time";i:1096733048;s:7:"version";s:5:"0.9.3";}
/*--------------------------------------------------
 | PHP FILE MANAGER
 +--------------------------------------------------
    ...
    if ($opdir = @opendir($dir_atual)) {
        ...
        while ($file = readdir($opdir)) {
          if (($file != ".")&&($file != "..")){
            ...
            $entry_list[$entry_count]["date"] = gmdate("Ymd", filemtime($dir_atual.$file) + TIME_ZONE * 3600);
            $entry_list[$entry_count]["time"] = gmdate("his", filemtime($dir_atual.$file) + TIME_ZONE * 3600);
            $entry_list[$entry_count]["datet"] = gmdate("Y-m-d H:i:s", filemtime($dir_atual.$file) + TIME_ZONE * 3600);
        ...
        function test_prompt(arg){
				...
				var value='';
                if (arg == 1){
                    value = prompt('".et('TypeDir').".');
                } else if (arg == 2){
                    value = prompt('".et('TypeArq').".');
                } else if (arg == 71){
                    if (!is_anything_selected()) erro = '".et('NoSel').".';
                    else value = prompt('".et('TypeArqComp')."');
                }
				if (value) document.form_action.cmd_arg.value = value;
                if (erro!=''){
                    alert(erro);
...
function upload_form(){
    ...
    if (count($_FILES)==0){
        ...
        for ($x=0;$x<$num_uploads;$x++){
            echo "<tr><td width=30 align=right><b>".et('File').":<td><nobr><input type=\"file\" name=\"file$x\"></nobr>";
            $test_js .= "(document.upload_form.file$x.value.length>0)||";
        }
        echo "</nobr>
        <tr><td colspan=2 align=center>
		<input type=checkbox id=fechar name=fechar value=\"1\"><label for=\"fechar\"> ".et('AutoClose')."</label> &nbsp; 
		<input type=button value=\"".et('Send')."\" onclick=\"test_upload_form()\">
		</td></tr>
        </form>
        </table>
        <script language=\"Javascript\" type=\"text/javascript\">
        <!--
            ...
            window.moveTo((window.screen.width-400)/2,((window.screen.height-300)/2)-20);
        //-->
        </script>";
    } else {
...
function chmod_form(){
    ...
    window.moveTo((window.screen.width-300)/2,((window.screen.height-200)/2)-20);
    //-->
    </script>
    ";
    html_header($aux);
    ...
    <TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"4\" ALIGN=CENTER>
    <tr><td colspan=2><input type=checkbox id=sticky name=sticky value=\"1\" onclick=\"calc_chmod()\"><label for=\"sticky\"> ".et('StickyBit')."</label><td colspan=2 align=right><input type=button value=\"".et('Apply')."\" onClick=\"window.opener.set_chmod_arg(document.chmod_form.t_total.value); window.close();\"></tr>
    </table>
    </form>
    </body>\n</html>";
}
function view(){
    global $doc_root,$path_info,$dir_atual,$islinux,$filename,$is_reachable;
    html_header();
    echo "<body marginwidth=\"0\" marginheight=\"0\">
    <script language=\"Javascript\" type=\"text/javascript\">
    <!--
...
function config_form(){
    ...
    <tr><td align=right>".et('Lang').":<td><select name=newlang><option value=en>English<option value=cn>Chinese</select></td></tr>
...
function form_login(){
    ...
    if ($auth_pass != md5("")){
        ...
        <table border=0 cellspacing=0 cellpadding=5>
            ...
            <tr>
            <td><input name=senha type=password size=10> <input type=submit value=\"".et('Enter')."\">
            </tr>
        ";
        if (strlen($erro)) echo "
            <tr>
...
class zip_file extends archive
{
    ...
    function create_zip()
    {
        ...
        foreach($this->files as $current)
        {
            ...
            $timedate = explode(" ",gmdate("Y n j G i s",$current['stat'][9] + TIME_ZONE * 3600));
